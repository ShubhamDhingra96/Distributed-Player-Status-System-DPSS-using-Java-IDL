...
public class Client {
	...
	public static void main(String[] args) throws NotFound, CannotProceed, InvalidName {
		...
		Player player =(Player) PlayerHelper.narrow(ncRef.resolve_str("Player"));
		...
		while (true) {
			...
			if (signInAs > 0 && signInAs < 3 && step < 99) {
				...
				if (signInAs == 1) {
					...
					while (true) {
						if (playerStep == 0) {
							System.out.println("\t1)Create Account\t2)SignIn\t3)SignOut\t4) Transfer Account");
							System.out.println();
					...
					while (true) {
						if (playerStep == 0) {
							System.out.println("\t1)SignIn\t2)Exit\t3)Suspend Account");
							System.out.println();
							playeraction = scanner.nextInt();
						}
						if (playeraction == 1) {
